...
#2. CAPTURAMOS O CREAMOS DATOS
datos = sns.load_dataset("tips")

datos2 = []

datos_aleatorios = np.random.RandomState(0)
ejex = datos_aleatorios.normal(0, 1, size = 1000)
...
#CON GRID
sns.set_style("whitegrid", {'grid.color': 'blue', 'grid.linestyle': '-.'})
sns.histplot(x = ejex, kde = True, color = "red", alpha=0.5)
plt.show()
...
plt.show()
    ...
    #REPRESETACIÓN POR PUNTOS
sns.scatterplot(x = "total_bill", y ="tip", data = datos).set_title("MODA DE COSTE Y PERSONAS POR PUNTOS")
plt.show()
...
plt.show()
    ...
    #DIAGRAMA DE CAJAS
sns.boxplot(x = "day", y = "total_bill", data = datos, hue = "day", legend = True)
plt.show()

    #DIAGRAMA DE VIOLINES
sns.violinplot(x = "day", y = "total_bill", data = datos, hue = "day", legend = True)
plt.show()

    #GRÁFICO DE ENJAMBRE
np.random.seed(8)
x = np.random.normal(0,1,1000)
y = 10 * x + 5 * np.random.normal(0,1,1000)
