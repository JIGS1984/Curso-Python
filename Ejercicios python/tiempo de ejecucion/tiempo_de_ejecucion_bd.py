...
# Función para almacenar la fecha de nacimiento en la base de datos SQLite
def almacenar_fecha_nacimiento(fecha):
    conn = sqlite3.connect('datos_usuario.db')
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS usuario
                 (id INTEGER PRIMARY KEY, fecha_nacimiento TEXT)''')
    c.execute("INSERT INTO usuario (fecha_nacimiento) VALUES (?)", (fecha.strftime("%Y-%m-%d"),))
    conn.commit()
    conn.close()
...
def main():
    ...
    if fecha_nacimiento is not None:
        almacenar_fecha_nacimiento(fecha_nacimiento)
    else:
